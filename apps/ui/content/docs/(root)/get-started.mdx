...
This guide provides the essentials for adding **ness ui** components to your React application.
...
For new projects, use the **ness style** preset to initialize everything in one command:
...
Components are styled with the design token system we use at **creantly.com**, which is defined by CSS variables and implemented with Tailwind CSS.
...
When you use `@ness/style`, `@ness/fonts` is installed automatically — Inter for `--font-sans` and `--font-heading`, Geist Mono for `--font-mono` — all wired in your `layout.tsx`.
...
**Note:** Next.js starters default to variable names like `--font-geist-sans`, which don't match ness's expected `--font-sans`. If fonts appear broken after setup, check that the variable names match.
...
- **[Agent Skills](/ui/docs/skills)** — Install ness knowledge directly into your AI assistant with `npx skills add nesscom/ness`. Covers all 53 primitives, styling conventions, migration patterns, and particle examples.
