---
name: ness
description: Helps implement ness UI components correctly. Use when building UIs with ness primitives (buttons, dialogs, selects, forms, menus, tabs, inputs, toasts, etc.), migrating from shadcn/Radix to ness/Base UI, composing trigger-based overlays, or troubleshooting ness component behavior. Covers imports, accessibility, Tailwind styling, and common pitfalls.
compatibility: Requires Tailwind CSS v4 and @base-ui/react. Designed for React projects using the ness component registry.
license: MIT
metadata:
  author: nesscom
---
...
ness ui is a component library built on Base UI with a shadcn-like developer experience plus a large particle catalog.

## What this skill is for

Use this skill to:

- pick the right ness primitive(s) for a UI task
- write correct ness usage code (imports, composition, props)
...
- ness components docs: `apps/ui/content/docs/components/*.mdx`
  - `https://github.com/nesscom/ness/tree/main/apps/ui/content/docs/components`
- ness particle examples: `apps/ui/registry/default/particles/p-*.tsx`
  - `https://github.com/nesscom/ness/tree/main/apps/ui/registry/default/particles`
- ness particles catalog: `https://ness.com/ui/particles`
- docs map for agents: `https://ness.com/ui/llms.txt`

## Out of scope

- Maintaining ness monorepo internals/build pipelines.
...
3. Follow ness naming and APIs from docs exactly.
4. Keep examples accessible and production-realistic.
5. Prefer concise code that mirrors ness docs/particles conventions.
6. Assume Tailwind CSS v4 conventions in ness examples and setup guidance.

## Critical usage rules

Always apply before returning ness code:

- Do not invent ness APIs. Verify against component docs first.
...
- Prefer styled ness exports first; use `*Primitive` exports only when custom composition/styling requires it.
...
- `./references/rules/migration.md` — shadcn/Radix to ness/Base UI migration patterns
...
3. Select primitives from ness docs first; avoid custom fallback unless needed.
...
```bash
npx shadcn@latest add @ness/<component>
```
...
- imports and props match ness docs
