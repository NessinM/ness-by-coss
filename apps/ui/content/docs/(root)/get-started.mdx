...
This guide provides the essentials for adding **creantly ui** components to your React application.
...
For new projects, use the **creantly style** preset to initialize everything in one command:
...
Components are styled with the design token system we use at **hookra.com**, which is defined by CSS variables and implemented with Tailwind CSS.
...
When you use `@creantly/style`, `@creantly/fonts` is installed automatically — Inter for `--font-sans` and `--font-heading`, Geist Mono for `--font-mono` — all wired in your `layout.tsx`.
...
**Note:** Next.js starters default to variable names like `--font-geist-sans`, which don't match creantly's expected `--font-sans`. If fonts appear broken after setup, check that the variable names match.
...
```tsx
import { Slider, SliderValue, SliderPrimitive } from "@creantly/ui/components/slider";
```

## Base UI Re-exports

When you only need `useRender`, `mergeProps`, `CSPProvider`, or `DirectionProvider`, import them from `@creantly/ui/base-ui/*` instead of adding `@base-ui/react` as a direct dependency:

- `@creantly/ui/base-ui/use-render`
- `@creantly/ui/base-ui/merge-props`
- `@creantly/ui/base-ui/csp-provider`
- `@creantly/ui/base-ui/direction-provider`

This is especially useful in **monorepos**: apps and packages that already depend on `@creantly/ui` can use these utilities without adding `@base-ui/react` as another dependency. You avoid duplicate installs, keep versions in sync, and simplify your dependency tree—`@base-ui/react` lives in one place (`@creantly/ui`), and everything else imports through it.
...
- **[Agent Skills](/ui/docs/skills)** — Install creantly knowledge directly into your AI assistant with `npx skills add nesscom/creantly`. Covers all 53 primitives, styling conventions, migration patterns, and particle examples.
