...
description: A practical guide for migrating from Radix-based libraries to ness ui components.
---

This guide is for developers who already have applications built on Radix-based libraries — frameworks that wrap Radix primitives — and **want to adopt ness ui**.

**ness ui** is built on top of **[Base UI](https://base-ui.com/)**, using a custom abstraction that stays intentionally close to its mental model. The component API exposed by ness ui is intentionally similar to shadcn/ui, which is why this guide also highlights the main differences between shadcn/ui and ness ui.

## Overview

**ness ui** uses **Base UI** internally, not Radix. This means the architecture and API patterns are different by design. However, we've worked to make the migration path as clear as possible by:
...
**Tip:** This migration guide is structured to be LLM-friendly. You can provide this page to AI coding assistants (like Claude, ChatGPT, etc.) to help automate the conversion of your Radix components to ness ui.
...
Not every component uses this pattern. Apply it where the ness component supports `render`.
...
So, if you want to preserve the original shadcn/ui badge size, you should use the `lg` size in ness ui.
...
ness ui button sizes are more compact compared to shadcn/ui, making them better suited for dense applications. We also introduce new sizes (`xs`, `xl`, `icon-sm`, `icon-lg`) for more granular control:

| Size      | Height (shadcn/ui) | Height (ness ui) |
| --------- | ------------------ | ---------------- |
| `xs`      | -                  | 24px             |
| `sm`      | 32px               | 28px             |
| `default` | 36px               | 32px             |
...
So, for example, if you were using the `default` size in shadcn/ui and you want to preserve the original height, you should use the `lg` size in ness ui.
...
So, if you want to preserve the original shadcn/ui input height (36px), you should use the `lg` size in ness ui.
...
| Component     | Radix UI Prop                                  | ness ui / Base UI Prop                      |
| ------------- | ---------------------------------------------- | ------------------------------------------- |
...
ness ui select sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
...
`alignItemWithTrigger` controls whether the popup aligns the selected item with the trigger text. In ness ui this prop is available on `SelectPopup` (legacy alias: `SelectContent`).
...
**Quick Checklist:**
- ness ui `Slider` uses Base UI's multiple value approach
...
ness ui toggle group sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
